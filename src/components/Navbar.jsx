    ...
    <header className="sticky top-0 z-40 w-full glass-panel border-b border-stone-200/80 dark:border-slate-800 transition-colors backdrop-blur-md bg-white/90 dark:bg-slate-900/90">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center space-x-2 sm:space-x-2.5">
          ...
          {/* Role Switcher */}
          <div className="hidden xl:flex items-center bg-stone-100 dark:bg-slate-800/90 p-1 rounded-xl border border-stone-200/60 dark:border-slate-700/60 text-[11px] font-bold">
            <button
          ...
          {userRole === 'admin' && user && (
            <button
              onClick={onOpenAdmin}
              className="hidden sm:flex items-center space-x-1.5 px-3 py-1.5 text-xs font-semibold text-white bg-amber-600 hover:bg-amber-700 rounded-lg shadow-sm shadow-amber-600/25 transition-all"
            >
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden border-t border-stone-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-4 space-y-3 animate-fadeIn">
          {/* Role Switcher */}
          <div className="flex bg-stone-100 dark:bg-slate-800 p-1 rounded-xl text-xs font-bold">
            <button
              onClick={() => setUserRole('student')}
              className={`flex-1 py-2 rounded-lg transition-all ${
                userRole === 'student' ? 'bg-amber-600 text-white shadow-sm' : 'text-slate-500 dark:text-slate-400'
              }`}
            >
              🎓 Student View
            </button>
            <button
              ...
              className={`flex-1 py-2 rounded-lg transition-all flex items-center justify-center space-x-1 ${
                userRole === 'admin' ? 'bg-amber-600 text-white shadow-sm' : 'text-slate-500 dark:text-slate-400'
              }`}
            >
          ...
          <div className="pt-2 border-t border-stone-100 dark:border-slate-800 space-y-2">
            ...
            {userRole === 'admin' && user && (
              <button
                ...
                className="w-full py-2.5 rounded-xl bg-amber-600 text-white font-bold text-xs flex items-center justify-center space-x-1.5 shadow-sm"
              >
